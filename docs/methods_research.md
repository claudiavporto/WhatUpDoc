...
Claudia Porto, Christopher Swartz, Sean Costello, Kat Fountain
...
1. **Document parsing** — extract text from PDF/DOCX while preserving location metadata (page numbers) for citation.
...
**Limitation noted:** the mock embedding captures only lexical overlap; it validates plumbing, not semantic retrieval quality. Semantic evaluation with real embeddings is Milestone 4 work.
...
All parameters live in `configs/config.yaml`; the privacy guard in `utils/helpers.py` refuses any non-localhost endpoint before the first network call.
